...
2. 后端 `question.py` 模型：所有请求/响应模型移除 required 字段

3. 后端 `survey_service.py`：`_resolve_question_refs` 移除 required 解析

4. 前端 `QuestionManager.tsx`：

- `handleDelete` 先查使用情况，confirm 中列出受影响问卷名称
- `VersionCard` 小字从"必答/选答"改为"创建者: xxx"
- 新版本表单移除"必答题"勾选框

5. 前端 `SurveyEditor.tsx`：选题面板版本显示改为创建者用户名；createNewVersion 调用移除 required

6. 前端 `types/index.ts`：QuestionVersion、CreateQuestionRequest、CreateVersionRequest 移除 required
...
针对需求六"修改时可以决定是否创建新版本"，目前的实现是修改题目时始终自动创建新版本，用户没有选择权。需要增加"在本版本修改"和"创建新版本"二选一的功能，同时保证已发布/已关闭问卷引用的版本不能被原地修改。

### result

**后端改动：**

1. `backend/app/models/question.py`：新增 `QuestionUpdateVersionRequest` 模型，用于原地更新指定版本内容的请求

2. `backend/app/services/question_service.py`：新增 `update_version()` 函数
   - 支持原地更新指定版本的 type/title/options/validation 内容
   - 安全检查：查找引用了该 question_ref_id + version_number 的问卷中 status 为 published 或 closed 的，如果存在则拒绝修改并返回友好错误提示
   - 如果没有被已发布/已关闭问卷使用，则通过 `$set` + 数组索引原子更新版本内容

3. `backend/app/routes/questions.py`：新增 `PUT /{question_id}/versions/{version_number}` 路由，调用 `update_version` 服务

**前端改动：**

4. `frontend/src/services/api.ts`：新增 `updateVersion()` API 调用函数，PUT 请求到 `/questions/{id}/versions/{vn}`

5. `frontend/src/components/QuestionManager.tsx`：
   - 将原来的"新版本"按钮改为"编辑"按钮
   - 点击"编辑"时智能判断当前最新版本的使用情况：
     - 如果最新版本**未被**已发布/已关闭问卷使用 → 弹出蓝色选择框，提供"✏️ 修改当前版本"和"＋ 创建新版本"两个选项，附说明文字
     - 如果最新版本**正在被**已发布/已关闭问卷使用 → 直接进入"创建新版本"模式，并显示橙色提示说明原因
   - 编辑表单标题根据模式显示"修改当前版本 (vN)"或"创建新版本"
   - 保存按钮根据模式调用 `updateVersion`（原地修改）或 `createNewVersion`（创建新版本）
   - 新增 `editFor`、`editMode`、`editChoiceFor` 三个状态管理编辑流程
...
在编辑已有问卷看已有题目的时候，能不能和编辑已有问卷添加新题目时一样，也有添加到题库的选项，并且已经这个题目在之前我已经添加入题库中，就一直显示已添加入题库，而不再出现添加到题库的选项。这样自然就保证了不能重复加入题库。

### result

修改 `frontend/src/components/SurveyEditor.tsx`：

1. **将 `bankedQids`（按问卷内局部 id 追踪）改为 `bankedRefIds`（按 `question_ref_id` 真实题目 id 追踪）**，使新建题目和已有题目使用统一的判断逻辑。

2. **加载问卷时额外调用 `getBankedQuestions()`**，获取用户题库列表，构建 `bankedRefIds` Set，用于判断每道题是否已在题库中。

3. **渲染逻辑统一**：不再区分 `newlyCreatedQids`，所有题目（无论是新建还是已有）都通过 `bankedRefIds.has(q.question_ref_id)` 判断——未在题库显示"📥 加入我的题库"按钮，已在题库显示"✅ 已添加入题库"。后端 `addToBank` 使用 `$addToSet`，天然幂等防重复。
...
现在我在编辑已有问卷里面新加题目+上移问题，每题目的序号和个数都完全混乱了，请修复。
...
- `addQuestion` 和 `addFromPicker` 中 `question_id` 的生成方式从 `` `q${order}` ``（基于当前题目数量，删除后再添加会产生重复 id，导致 React key 冲突和渲染混乱）改为 `` `q_${Date.now()}_${Math.random().toString(36).slice(2, 6)}` ``（基于时间戳+随机数，保证全局唯一）
- `question_id` 仅作为问卷编辑器内部的局部标识符（React key、跳转逻辑目标、状态 Map 的 key），不依赖具体格式，改为唯一 id 后所有功能正常工作
- 显示的题号 `Q{n}` 始终按数组下标 `qi + 1` 计算，不受 id 格式影响
...
运行 `pytest backend/tests/ -v` 后出现 8 个失败，报错为 `FakeCollection.find() takes from 1 to 2 positional arguments but 3 were given`，并增加对两点新增需求的测试：

1. 在编辑已有问卷查看已有题目时，也显示"添加到题库"按钮；已在题库中则显示"已添加入题库"，防止重复加入。
2. 版本编辑采用智能折中：若该版本未被已发布/已关闭问卷使用，则允许"在本版本修改"；否则仅允许"创建新版本"。

### result

本次完成了后端修复与测试补强，核心落地如下：

1. **修复测试桩兼容问题（根因修复）**
   - 修改 `backend/tests/conftest.py`：
     - `FakeCollection.find` 新增 `projection` 参数支持，兼容 `find(filter, projection)` 调用。
     - 增加 `$elemMatch` 匹配支持，用于版本保护场景查询。
     - 增强点路径读写能力，支持数组下标路径（如 `versions.0.title`）以适配版本原地更新。

2. **补充“版本智能判断”自动化测试**
   - 修改 `backend/tests/test_questions.py`，新增 4 个用例：
     - `test_update_version_in_place_should_succeed_when_only_draft_surveys_reference`
     - `test_update_version_in_place_should_fail_when_published_survey_references`
     - `test_update_version_in_place_should_fail_when_closed_survey_references`
     - `test_protected_version_should_allow_create_new_version`

3. **修正删除题目保护逻辑**
   - 修改 `backend/app/services/question_service.py`：
     - 若题目被已发布/已关闭问卷引用，则删除时返回 `QUESTION_IN_USE`（4003），禁止删除。
     - 仅在无受保护引用时，才允许删除并从相关草稿问卷中移除引用。
