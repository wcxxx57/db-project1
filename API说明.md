...
| 版本管理 | PUT    | `/questions/{question_id}/versions/{version_number}`         | 需要 JWT | 原地更新指定版本（仅当未被已发布/已关闭问卷引用且为创建者） |
...
- `access_code`：问卷公开访问码，用于生成分享链接和公开填写入口
- `question_ref_id`（题目谱系 ID）：独立题目的全局唯一标识，即 MongoDB `ObjectId`（用于跨问卷引用与版本管理）
- `question_id`（问卷内局部题号）：问卷内部的局部编号，如 `q1`、`q2`，用于跳转逻辑路由（仅在问卷上下文中有效）
...
- 题目的共享/取消共享/删除操作仅创建者可执行。
- 题库操作（加入/移出）：创建者和被共享者均可执行。
- 版本编辑权限：共享给我的题目不能原地修改（in-place update）；被共享者可基于原题创建新版本。只有题目创建者在且该版本未被任何已发布或已关闭的问卷引用时，才允许使用 `PUT /questions/{question_id}/versions/{version_number}` 对该版本进行原地更新。
