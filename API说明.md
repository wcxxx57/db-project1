...
- 响应体中的 `code` 为业务码：成功固定为 `0`，失败使用第 8 节业务错误码（如 1001/2001/3001）。
...
- `published` 状态的问卷不可编辑，必须先关闭问卷
- `draft` 或 `closed` 状态的问卷可以编辑
- 所有字段均为可选，仅更新提供的字段

**⚠️ 注意事项**：

- 修改题目结构（删除题目、修改选项等）可能导致历史答卷统计不准确
- 如果问卷已有答卷数据（`response_count > 0`），建议：
  - 只修改标题、说明、截止时间等元数据
  - 避免修改 `questions` 数组（删除题目、修改选项 ID 等）
  - 如需大幅修改题目结构，建议创建新问卷
- 系统统计功能已实现容错逻辑：
  - 会自动忽略找不到定义的题目和选项
  - 在统计结果中添加警告信息提示数据不完整
...
说明：第一阶段统计接口默认返回全量数据（包括 `text_responses` 与 `number_statistics.values`）。若数据量较大，可在后续阶段扩展分页、采样或导出能力。
